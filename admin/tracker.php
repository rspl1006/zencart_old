...
// END TY1 ------------------------------------------------------------------
		...
		if($status == 8){
			...
			$request = curl_init(WITECH_ADMIN_URL . '/wiTechAdmin/DispatchServlet?Action=finalizeOrder&OrderId=' . (int)$oID);  
...
// END TY3 ----------------------------------------------------------------
	  ...
	  case 'manual_add':
		if (isset($_GET['oID'])) {
			...
			$adjust_text = $_GET['adjust_text'];
			if(is_numeric($adjust_amount) && $adjust_text != '') { 
				echo "Adjustment amount is: ". $adjust_amount; 
				setlocale(LC_MONETARY, 'en_US');
				$sql_data_array = array('orders_id' => $_GET['oID'],
										'title' => "Adjustment - " . $adjust_text .":",
										'text' => money_format('%.2n', $adjust_amount),
										'value' => round($adjust_amount,4),
										'sort_order' => 900,
										'class' => 'ot_manual');
				zen_db_perform(TABLE_ORDERS_TOTAL, $sql_data_array);
			}

			$db->Execute("delete from " . TABLE_ORDERS_TOTAL . " where orders_id = " . (int)$_GET['oID'] . " and class = 'ot_total'");
			
			$order_total_rec = $db->Execute("select sum(value) as amount from zen_orders_total where orders_id = " . $_GET['oID']);
			$order_total_amount = round($order_total_rec->fields["amount"],4);
			
			$sql_data_array = array('orders_id' => $_GET['oID'],
										'title' => "Total:",
										'text' => money_format('%.2n', $order_total_amount),
										'value' => $order_total_amount,
										'sort_order' => 999,
										'class' => 'ot_total');
			zen_db_perform(TABLE_ORDERS_TOTAL, $sql_data_array);
			
			$db->Execute("update zen_orders z set z.order_total = " . $order_total_amount ." where z.orders_id = " . $_GET['oID']);

			zen_redirect(zen_href_link(FILENAME_TRACK,'oID=' . $_GET['oID'] . '&' .  'action=edit', 'NONSSL'));
		}
		break;
    }
  }
  ...
  if (($action == 'edit') && isset($_GET['oID'])) {
	...
	$portal = $db->Execute("SELECT u.USER_IDENT,a.NAME
                              FROM zen_orders z, zen_customers c, witech_dp_user u, account_type a
                             WHERE z.customers_id = c.customers_id
                               AND u.id = c.customers_witech_id
                               AND u.ACCOUNT_TYPE_ID = a.id
                               AND z.orders_id = '" . (int)$oID . "'");
	$portalAccount = $portal->fields['USER_IDENT'];
	$accountType = $portal->fields['NAME'];
  }
...
<?php
	...
	if($order_exists == true){
		$review_query = "SELECT description FROM zen_orders_review r, zen_orders_review_reason rr WHERE rr.orders_review_reason_id = r.orders_review_reason_id AND r.orders_review_order_id = " . $oID . " ORDER BY rr.orders_review_reason_id;";
		$review_reasons = $db->Execute($review_query);
		
		if($review_reasons->RecordCount() > 0){

			echo "<div style='padding: 5px;'><b>Review Reasons:</b><br/><ol class='order-review'>";
			
			 while (!$review_reasons->EOF) {
				echo "<li>" . $review_reasons->fields['description'] . "</li>";
				$review_reasons->MoveNext();		
			}
			echo "</ol></div><hr />";
		}
	}
  ...
  if (($action == 'edit') && ($order_exists == true)) {
    ...
    if ($order->info['payment_module_code']) {
      if (file_exists(DIR_FS_CATALOG_MODULES . 'payment/' . $order->info['payment_module_code'] . '.php')) {
        ...
        if (file_exists(DIR_FS_CATALOG_LANGUAGES . $_SESSION['language'] . '/modules/payment/' . $order->info['payment_module_code'] . '.php')) {
      ...
      <tr>
        <td><table width="100%" border="0" cellspacing="0" cellpadding="2">
          ...
          <tr>
			...
			<tr>
                <td class="main"><b><?php echo 'Portal Account:'; ?></b></td>
                <td class="dataTableContent"><?php
                  echo $portalAccount . '&nbsp;'; ?></font></td>
              </tr>
              <tr>
                <td class="main"><b><?php echo 'Account Type:'; ?></b></td>
                <td class="dataTableContent"><?php
                  echo $accountType . '&nbsp;'; ?></font></td>
              </tr>
             <tr>
...
?>
			...
			<tr>
				<td align="right" colspan="8">
					<br/>
					<?php	
					echo zen_draw_form('manual_adjust', FILENAME_TRACK, '', 'get', '', true);
					echo zen_draw_hidden_field('oID', $oID);
					echo zen_draw_hidden_field('action', 'manual_add');
					echo "Adjustment Amount: ";
					echo zen_draw_input_field('adjust_amount',  '');
					echo "<br/>";					
					echo "Adjustment Reason: ";
					echo zen_draw_input_field('adjust_text',  ''); 
					echo "<br/>";					
					echo "<input type='submit' value='Adjust' />";
					echo '</form>';
					?>
				</td>
			</tr>
